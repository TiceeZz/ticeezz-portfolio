const skills = [
  {
    category: 'Visual & Spatial Design',
    tag: '+',
    className: 'colTools',
    items: [
      { name: 'Figma / Adobe Suite', sub: 'UI & Graphic', level: '95%' },
      { name: '3Ds Max / Rhino', sub: '3D Modeling', level: '85%' },
      { name: 'CGI & Video Editing', sub: 'Visual Delivery', level: '90%' },
      { name: 'Procreate', sub: 'Concept Sketch', level: '80%' },
    ],
  },
  {
    category: 'AI Engineering & Gen',
    tag: '+',
    className: 'colAi',
    items: [
      { name: 'ComfyUI / SD', sub: 'Node Workflow', level: '90%' },
      { name: 'Seedance / Runway', sub: 'AIGC Video', level: '85%' },
      { name: 'Midjourney', sub: 'Image Gen', level: '95%' },
      { name: 'LLM / Prompt Eng.', sub: 'AI Agent / RAG', level: '88%' },
    ],
  },
  {
    category: 'Experience Architecture',
    tag: '+',
    className: 'colUx',
    items: [
      { name: 'Service Design (PSS)', sub: 'System', level: '95%' },
      { name: 'Journey / 2.5D Flow', sub: 'Mapping', level: '90%' },
      { name: 'Persona / Empathy', sub: 'User Research', level: '92%' },
      { name: 'Accessibility Design', sub: 'Inclusive UX', level: '85%' },
    ],
  },
];

export default skills;
